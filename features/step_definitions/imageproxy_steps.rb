...
Then /^proxying "([^"]*)" should work$/ do |url|
  ...
  assert open("http://127.0.0.1:#{configatron.nodejs.port}/image_proxy?url=#{url}").content_type.match(/image/)
end
...
#cleanup
  ...
  assert open("http://127.0.0.1:#{configatron.nodejs.port}/image_proxy?height=100&width=100&url=#{url}").content_type.match(/image/)
end
