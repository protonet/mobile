  ...
  protected
    def get_user params
      if params.include?('session_id')
        # session_id given
        begin
          session = Marshal.load(Base64.decode64(params['session_id']).split('--').first)
        rescue
        ...
        user = User.find_by_id(params['user_id'])
        return nil if !user || !user.communication_token_valid?(params['token'])
      end
      user
    end
