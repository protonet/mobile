...
(function (document) {
	var a = -1,
		...
		newDocumentCreateElement = function (node) { // console.log('createElement, ' + node);
			// Don't apply opc-hack for <input> elements
			// Otherwise this (which is used in recent jQuery version) won't work anymore:
			// 		var input = document.createElement("input");
			// 		input.setAttribute("type", "radio"); // BOOM! Element is in a DocumentFragment
			if (node.match(inputRegExp)) {
				return documentCreateElement(node);
			}
	...
	function opc () {
		...
		if (el[length] && el[length] != arguments.callee.elementsLength && (arguments.callee.elementsLength = el[length])) { // console.log('elementChange, ' + instance.nodeName);
			while (++a < el[length]) {
				try { el[a].cloneNode = newCloneNode; } catch(e) {}
			}
		}
	};
