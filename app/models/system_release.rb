class SystemRelease
  ...
  class << self
    ...
    def update!(password=nil, release_version=nil)
      ...
      babushka_update     = system( "bash -l -c \"#{release_version_var} export HISTIGNORE=\"*ptn_babushka_update*\"; #{configatron.current_file_path}/script/ptn_babushka_update #{license_key}\""
      )
      babushka_migrations = babushka_update && system( "bash -l -c \"export HISTIGNORE=\"*ptn_babushka_migrations*\"; #{configatron.current_file_path}/script/ptn_babushka_migrations '#{password}'\""
      )
      deployer_update     = babushka_migrations && system( "bash -l -c \" #{release_version_var} export HISTIGNORE=\"*ptn_deployer_update*\"; #{configatron.current_file_path}/script/ptn_deployer_update '#{license_key}'\""
      )
      release_update      = deployer_update && system( "bash -l -c \" #{release_version_var} #{configatron.current_file_path}/script/ptn_release_update #{configatron.shared_file_path}\""
      )
