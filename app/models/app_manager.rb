module AppManager
  ...
  class AppProtonetBinding
    ...
    class AppPathNotSpecified < StandardError; end
    ...
    def initialize(configuration)
      ...
      raise AppPathNotSpecified if configuration[:app_path].blank?
      ...
      self.app_host = configuration[:app_host]
      self.app_port = configuration[:app_port]
    end
    ...
    def app_url
      host = local? ? SystemPreferences.public_host.split(':').first : app_host
      host_with_port = app_port.to_i == 80 ? host : "#{host}:#{app_port}"
